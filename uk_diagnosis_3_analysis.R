...
data_dir_csv  <- file.path("data", "uk", "diagnosis_csv_3")
if (!dir.exists(data_dir_csv)) {
  dir.create(data_dir_csv, recursive = TRUE, showWarnings = FALSE)
}
out_dir_report  <- file.path("output", "uk", "diagnosis_report_3")
...
convert_diagnosis_file <- function(year_range) {
  ...
  df_raw <- read_excel(
    path      = xlsx_path,
    sheet     = "All Diagnoses 3 Character",
    col_names = FALSE
  )
  ...
  header_row_idx <- which(
    df_raw[[1]] == "All diagnoses: 3 character code and description"
  )[1]
